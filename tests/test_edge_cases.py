...
def _pytest_setup():
    """Pytest fixture: run setup before edge tests."""
    setup()
    yield
    teardown()
...
def test_edge_jids():
    ...
    try:
        ...
        assert FAIL == 0, f"edge_jids: {FAIL} assertion(s) failed"
...
def _run_edge_jids():
    ...
    T("jids", "normalize_jid(None) — no crash", isinstance(normalize_jid(None), str))  # returns "", not "None"
    ...
    T("jids", "normalize_text con emojis", normalize_text("Hola 😀") == "hola 😀")  # emojis preserved
    T("jids", "normalize_text con símbolos", normalize_text("¡Hola!") == "¡hola!")  # ¡ preserved
    ...
    T("jids", "jid_match con @lid — no match esperado", jid_match("34600000000", "212725569433687@lid") == False)  # LID != phone
...
def test_edge_rbac():
    ...
    try:
        ...
        assert FAIL == 0, f"edge_rbac: {FAIL} assertion(s) failed"
...
def _run_edge_rbac():
    ...
    T("rbac", "get_role_config — rol inexistente", isinstance(cfg, dict))
...
def test_edge_tool_guard():
    global PASS, FAIL, WARN
    PASS = 0; FAIL = 0; WARN = 0
    try:
        _run_edge_tool_guard()
    finally:
        assert FAIL == 0, f"edge_tool_guard: {FAIL} assertion(s) failed"

def _run_edge_tool_guard():
...
def test_edge_contacts():
    global PASS, FAIL, WARN
    PASS = 0; FAIL = 0; WARN = 0
    try:
        _run_edge_contacts()
    finally:
        assert FAIL == 0, f"edge_contacts: {FAIL} assertion(s) failed"

def _run_edge_contacts():
...
def test_edge_webhook():
    global PASS, FAIL, WARN
    PASS = 0; FAIL = 0; WARN = 0
    try:
        _run_edge_webhook()
    finally:
        assert FAIL == 0, f"edge_webhook: {FAIL} assertion(s) failed"

def _run_edge_webhook():
...
def test_edge_orchestrator():
    ...
    PASS = 0; FAIL = 0; WARN = 0
    try:
        _run_edge_orchestrator()
    finally:
        assert FAIL == 0, f"edge_orchestrator: {FAIL} assertion(s) failed"

def _run_edge_orchestrator():
...
def test_edge_agenda():
    global PASS, FAIL, WARN
    PASS = 0; FAIL = 0; WARN = 0
    try:
        _run_edge_agenda()
    finally:
        assert FAIL == 0, f"edge_agenda: {FAIL} assertion(s) failed"

def _run_edge_agenda():
    ...
    try:
        ...
        pass  # sys.exit(0) is expected for invalid input
    T("agenda", "auto_schedule tiempo vacío — no crash", safe)


def test_edge_alerts():
    global PASS, FAIL, WARN
    PASS = 0; FAIL = 0; WARN = 0
    try:
        _run_edge_alerts()
    finally:
        assert FAIL == 0, f"edge_alerts: {FAIL} assertion(s) failed"

def _run_edge_alerts():
...
def test_edge_concurrency():
    ...
    PASS = 0; FAIL = 0; WARN = 0
    try:
        _run_edge_concurrency()
    finally:
        assert FAIL == 0, f"edge_concurrency: {FAIL} assertion(s) failed"

def _run_edge_concurrency():
...
def test_edge_install():
    global PASS, FAIL, WARN
    PASS = 0; FAIL = 0; WARN = 0
    try:
        _run_edge_install()
    finally:
        assert FAIL == 0, f"edge_install: {FAIL} assertion(s) failed"

def _run_edge_install():
    ...
    # V2.0-F7: check_write_permission removed from codebase
    if os.geteuid() != 0:  # skip if running as root
        T("install", "os.access /root (read-only)", os.access("/root", os.W_OK) == False)
...
def test_edge_souls():
    ...
    try:
        ...
        assert FAIL == 0, f"edge_souls: {FAIL} assertion(s) failed"
...
def _run_edge_souls():
    ...
    T("souls", "load_soul_text JID inexistente — returns None", txt is None)  # None = no soul

    # Soul file corrupta
    (SOULS_DIR / "_default.md").write_text("")
    txt = load_soul_text("99999999999", {})
    T("souls", "load_soul_text default vacía — returns None", txt is None)  # no default soul
...
def test_edge_inbox():
    ...
    try:
        ...
        assert FAIL == 0, f"edge_inbox: {FAIL} assertion(s) failed"
...
def _run_edge_inbox():
    ...
    try:
        ...
        pass  # sys.exit() is expected for corrupted state
    T("inbox", "cmd_listar con entrada corrupta", True)  # exception is expected handling
...
def test_edge_files_send():
    global PASS, FAIL, WARN
    PASS = 0; FAIL = 0; WARN = 0
    try:
        _run_edge_files_send()
    finally:
        assert FAIL == 0, f"edge_files_send: {FAIL} assertion(s) failed"

def _run_edge_files_send():
...
def test_edge_knowledge():
    global PASS, FAIL, WARN
    PASS = 0; FAIL = 0; WARN = 0
    try:
        _run_edge_knowledge()
    finally:
        assert FAIL == 0, f"edge_knowledge: {FAIL} assertion(s) failed"

def _run_edge_knowledge():
...
def test_edge_dlp():
    ...
    try:
        ...
        assert FAIL == 0, f"edge_dlp: {FAIL} assertion(s) failed"
...
def test_edge_injection():
    ...
    try:
        ...
        assert FAIL == 0, f"edge_injection: {FAIL} assertion(s) failed"

def _run_edge_injection():
