...
def main():
    ...
    try:
        ...
        subprocess.run([hermes_cmd, "hooks", "add", "wa_inbox", "--command", f"python3 '{hook}'", "--event", "message_received"], capture_output=True)
        subprocess.run([hermes_cmd, "hooks", "add", "wa_inbox_alt", "--command", f"python3 '{hook}'", "--event", "whatsapp:message"], capture_output=True)
        print("✅ Skills deployed and inbox hooks registered.")
