...
import sys, json, urllib.request, urllib.parse, re, time, hashlib, unicodedata
...
def load_env():
    ...
    global BRIDGE
    BRIDGE = env.get("WHATSAPP_BRIDGE_URL", BRIDGE)
    return env
...
def obtener_grupos():
    ...
    # 1. Try Bridge first (Real-time)
    try:
        with urllib.request.urlopen(f"{BRIDGE}/groups", timeout=3) as r:
