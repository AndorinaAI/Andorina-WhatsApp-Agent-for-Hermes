# 📖 Andoriña v1.0.2-hotfix2 — Complete User Guide
...
- `scripts/bridge_health.py`: The auto-repair "medic" for the infrastructure. Creates a backup of `bridge.js` before patching.
- `scripts/setup_autostart.py`: Configures autostart on login (auto-detects terminal: GNOME, KDE, XFCE, MATE, LXDE, xterm).
- `scripts/setup_portable.py`: Downloads portable Qdrant binary (auto-detects x86_64/ARM64 architecture).
...
- **Bridge broken after patch:** Restore the backup: `cp bridge_andorina_bak.js bridge.js` in the `whatsapp-bridge/` directory, then `hermes gateway restart`.
...
- `scripts/bridge_health.py`: El "médico" de auto-reparación de la infraestructura. Crea copia de seguridad de `bridge.js` antes de parchear.
- `scripts/setup_autostart.py`: Configura el arranque automático al iniciar sesión (detecta terminal: GNOME, KDE, XFCE, MATE, LXDE, xterm).
- `scripts/setup_portable.py`: Descarga binario portable de Qdrant (detecta arquitectura x86_64/ARM64).
...
- **Bridge roto tras parcheo:** Restaura la copia de seguridad: `cp bridge_andorina_bak.js bridge.js` en el directorio `whatsapp-bridge/`, luego `hermes gateway restart`.
