...
def _detect_public_url():
    ...
    # 1. Env var set by user — most reliable, always wins
    url = os.environ.get("ANDORINA_WEBHOOK_URL", "").strip().rstrip("/")
    if url:
        return url, "env"
    # 2. Named Cloudflare tunnel config (~/.cloudflared/config.yml)
    try:
        import re as _re
        ...
        pass
    # 3. Quick tunnel active in memory (tunnel module keeps active_url global)
    try:
        sys.path.insert(0, str(SCRIPTS_DIR))
        from utils import tunnel as _tunnel_mod
        if _tunnel_mod.active_url:
            return _tunnel_mod.active_url, "cloudflare-quick"
    except Exception:
        pass
    # 4. Quick tunnel URL from tunnel.log (survives server restarts)
    try:
        import re as _re2
        tunnel_log = STATE_DIR / "tunnel.log"
        if tunnel_log.exists():
            text = tunnel_log.read_text(errors="ignore")
            m = _re2.search(r'https://[a-zA-Z0-9\-]+\.trycloudflare\.com', text)
            if m:
                return m.group(0), "cloudflare-quick"
    except Exception:
        pass
    # 5. Fallback to localhost (GUI server port)
...
class APIHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # ── Alerts ──
            ...
            stable = source in ("env", "cloudflare-named")
            self.send_json({"ok": True, "url": url, "source": source, "stable": stable})
