...
const App = {
  ...
  async loadDashboard(silent = false) {
    ...
    // Hindsight warning banner
    const hw = document.getElementById('hindsight-warning');
    if (hw) {
      if (!d.hindsight_running) {
        hw.style.display = '';
        const installed = hw.querySelector('.hind-installed');
        const notInstalled = hw.querySelector('.hind-not-installed');
        if (installed) installed.style.display = d.hindsight_installed ? '' : 'none';
        if (notInstalled) notInstalled.style.display = d.hindsight_installed ? 'none' : '';
      } else {
  ...
  async startHindsight() {
    const r = await this.post('hindsight/start', {});
    if (r.ok) {
      this.toast('🧠 Long-term memory started!', 'ok');
      this.loadDashboard(true);
    } else {
      this.toast('❌ ' + (r.output || r.error || 'Could not start memory'), 'error');
    }
  },
  ...
  async _renderWebhooksList() {
    ...
    // ── URL stability banner ──────────────────────────────────────────────────
    const urlData = await this.get('webhooks/url');
    this._webhookBaseUrl = urlData.url || '';
    const urlSource = urlData.source || 'local';
    const urlBanner = this.$('webhook-url-banner');
    const urlBannerText = this.$('webhook-url-banner-text');
    const formSection = this.$('webhook-form-section');
    ...
    if (urlBanner && urlBannerText) {
      if (urlSource === 'local') {
        ...
        urlBannerText.innerHTML = this.lang === 'en'
          ...
          <code>ANDORINA_WEBHOOK_URL=https://tu-url-publica</code> en la Configuración.</span>`;
    ...
    const base = this._webhookBaseUrl;
  ...
  editJID(jid) {
    ...
    this.get('guard/rules').then(d => {
      ...
      // Defer soul assignment to ensure the dropdown options are already rendered
      const _soulVal = e.custom_soul || '';
      requestAnimationFrame(() => {
        this.$('rbac-soul').value = _soulVal;
        // If value didn't stick (option not found), force it by adding a temp option
        if (this.$('rbac-soul').value !== _soulVal && _soulVal) {
          const opt = document.createElement('option');
          opt.value = _soulVal;
          opt.textContent = _soulVal;
          this.$('rbac-soul').appendChild(opt);
          this.$('rbac-soul').value = _soulVal;
        }
      });
