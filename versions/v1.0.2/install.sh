...
AGENT_DIRS=($(find "$HOME" -maxdepth 1 -type d -name ".*" -exec test -d "{}/skills" \; -print 2>/dev/null))
PROFILE_NAME="hermes"

if [ ${#AGENT_DIRS[@]} -gt 0 ]; then
    echo -e "   ${C_WHITE}🤖 Detected Agents / Agentes Detectados:${C_RESET}"
    for i in "${!AGENT_DIRS[@]}"; do
        name=$(basename "${AGENT_DIRS[$i]}")
    ...
    if [[ "$AGENT_CHOICE" != "0" ]] && [[ "$AGENT_CHOICE" =~ ^[0-9]+$ ]] && [ "$AGENT_CHOICE" -le ${#AGENT_DIRS[@]} ]; then
        idx=$((AGENT_CHOICE-1))
        name=$(basename "${AGENT_DIRS[$idx]}")
        PROFILE_NAME=${name#.}
    else
        echo -ne "   ${C_WHITE}✏️  Type Agent Profile Name / Escribe el Nombre del Perfil: ${C_RESET}"
        read -r PROFILE_NAME
        PROFILE_NAME=${PROFILE_NAME:-hermes}
    fi
else
    echo -ne "   ${C_WHITE}🤖 Agent Profile Name / Nombre del Perfil (e.g., hermes, stephanie) [hermes]: ${C_RESET}"
    read -r PROFILE_NAME
    PROFILE_NAME=${PROFILE_NAME:-hermes}
fi

if [[ "$PROFILE_NAME" == "hermes" || "$PROFILE_NAME" == "main" ]]; then
    export HERMES_HOME="$HOME/.hermes"
    export HERMES_CMD="hermes"
else
    export HERMES_HOME="$HOME/.${PROFILE_NAME}"
    export HERMES_CMD="$PROFILE_NAME"
fi
