# 🌟 Andoriña v1.0.2 — Complete Feature Reference
## 🇬🇧 English | 🇪🇸 Español
...
- **Auto-detection in installer:** `install.sh` scans `$HOME` for agent profiles and presents a selection menu.
...
- **`bridge_health.py`:** The main "medic". Runs automatically on every send attempt.
  ...
  - Kills zombie processes on the bridge port via `fuser -k`.
...
- **`setup_autostart.py`:** Creates a `.desktop` entry in `~/.config/autostart/` that starts the agent in a `gnome-terminal` window on login.
...
- **Auto-detección en el instalador:** `install.sh` escanea `$HOME` en busca de perfiles de agente y muestra un menú de selección.
...
- **`bridge_health.py`:** El "médico" principal. Se ejecuta automáticamente en cada intento de envío.
  ...
  - Mata procesos zombies en el puerto del puente mediante `fuser -k`.
...
- **`setup_autostart.py`:** Crea una entrada `.desktop` en `~/.config/autostart/` que arranca el agente en una ventana de `gnome-terminal` al iniciar sesión.
...
---
*Developed with ❤️ by Jorge & Antigravity. — Andoriña v1.0.2*
