# 🌟 Andoriña v1.5.2-Beta5 — Complete Feature Reference
## 🇬🇧 English | 🇪🇸 Español
...
## 🔧 What's New in v1.5.2-Beta5 (Bug Fix Release)
## 🔧 Novedades en v1.5.2-Beta5 (Versión de Corrección de Errores)

### 🇺🇸 English

| # | Bug / Feature | Files Changed |
|:---|:---|:---|
| 🐛 | **Group→Group Semantic Alerts** — `webhook.py` `pre_llm_call` path was building `chat_id` from the individual sender JID instead of the group JID, so alert rules with a group source never fired. Fixed by parsing `session_key` (`whatsapp:group:<GROUP_JID>[:<SENDER>]`) to extract the real group `@g.us` JID. Also passes `chat_name` for human-readable alert labels. | `webhook.py` |

### 🇪🇸 Español

| # | Error / Mejora | Archivos Modificados |
|:---|:---|:---|
| 🐛 | **Alertas Semánticas Grupo→Grupo** — El path `pre_llm_call` de `webhook.py` construia `chat_id` a partir del JID del miembro individual en lugar del JID del grupo. Las reglas de alerta con origen de grupo nunca se disparaban. Corregido parseando `session_key` (`whatsapp:group:<JID_GRUPO>[:<EMISOR>]`) para extraer el JID real `@g.us`. También pasa `chat_name` para labels legibles en las notificaciones. | `webhook.py` |

---

## 🔧 What's New in v1.5.2-Beta4 (Bug Fix Release)
## 🔧 Novedades en v1.5.2-Beta4 (Versión de Corrección de Errores)

### 🇺🇸 English

| # | Bug / Feature | Files Changed |
|:---|:---|:---|
| 🐛 | **RBAC JID suffix match** — `resolve_role()` in `rbac.py` now maps user JIDs using suffix matching to support country-code-less or short numbers. | `rbac.py` |
| 🐛 | **Alert source JID match** — Replaced unsafe substring check (`in`) with exact JID suffix match to prevent false alarm source triggers. | `alerts.py` |
| 🐛 | **Away admin exclusion** — Comparisons against `ADMIN_PHONE` now support suffix matching for away message exclusions. | `webhook.py` |
| 🐛 | **Webhook URL detection** — Webhook public URL automatic detection overhauled with multi-tier fallbacks and a stability status indicator. | `server.py` |
| ✨ | **Fuzzy keyword alerts** — Added accent and case-insensitive matching (`fuzzy_keyword_match`) for dynamic alert keywords. | `webhook.py` |
| ✨ | **Webhook stability banner** — Displays warning banner on dashboard UI if webhook URL is missing, using local hostnames, or unstable. | `server.py`, `app.js`, `index.html`, `style.css` |
| ✨ | **JID normalizer helper** — Centralized `_jid_match` and `_norm_jid` helper functions. | `alerts.py`, `webhook.py` |

### 🇪🇸 Español

| # | Error / Mejora | Archivos Modificados |
|:---|:---|:---|
| 🐛 | **Coincidencia de sufijos RBAC** — `resolve_role()` en `rbac.py` ahora coincide JIDs mediante sufijos para admitir números cortos o sin código de país. | `rbac.py` |
| 🐛 | **Coincidencia JID en alertas** — Reemplazada coincidencia insegura de subcadena (`in`) con coincidencia limpia de sufijo JID en origen de alertas. | `alerts.py` |
| 🐛 | **Exclusión admin en modo ausente** — Comparaciones con `ADMIN_PHONE` ahora soportan coincidencia de sufijo para auto-respuesta de de ausencia. | `webhook.py` |
| 🐛 | **Autodetección de URL webhook** — Detección automática de URL pública de webhook mejorada con fallbacks multinivel y estado de estabilidad. | `server.py` |
| ✨ | **Alertas semánticas difusas** — Añadida coincidencia insensible a acentos, mayúsculas y plurales (`fuzzy_keyword_match`) en palabras clave. | `webhook.py` |
| ✨ | **Banner de estabilidad webhook** — Muestra banner de advertencia si la URL de webhook está vacía, usa localhost o es inestable. | `server.py`, `app.js`, `index.html`, `style.css` |
| ✨ | **Ayudantes de normalización JID** — Funciones de ayuda centralizadas `_jid_match` y `_norm_jid`. | `alerts.py`, `webhook.py` |

---

## 🔧 What's New in v1.5.2-Beta3 (Bug Fix Release)
## 🔧 Novedades en v1.5.2-Beta3 (Versión de Corrección de Errores)
...
- **Beta-Safe Parsing:** Uses semantic regex to parse version tags like `1.5.2-Beta3` — non-numeric suffixes never break the comparison.
...
- **Soporte de Etiquetas Beta:** Usa regex semántico para parsear versiones como `1.5.2-Beta3` sin errores.
