    ...
    try:
        ...
        if event == "pre_llm_call":
            ...
            if jid:
                ...
                # Reserved soul names — treated as explicit "use Hermes base SOUL.md" (blocks all fallbacks)
                _RESERVED_SOULS = {"__HERMES__", "__DEFAULT__", "__NONE__"}
                _raw_soul = jid_entry.get("custom_soul", "")
                _hermes_native = (_raw_soul or "").strip() in _RESERVED_SOULS
                if plugin_name in _RESERVED_SOULS:
                    plugin_name = ""

                # Fallback: global_default_soul ONLY if no soul assigned AND not Hermes native AND not owner
                _effective_soul_name = plugin_name
                if not plugin_name and not _hermes_native and not is_owner(jid, env):
            ...
            if tool_name and not tool_name.startswith("plugin_"):
                ...
                if cmd and jid:
                    ...
                    if "@" not in jid and dm_mode == "game" and jid_entry.get("dm_game"):
                        ...
                        plugin_name = jid_entry.get("custom_soul", "").strip()
                    _RESERVED_SOULS_TC = {"__HERMES__", "__DEFAULT__", "__NONE__"}
                    if plugin_name in _RESERVED_SOULS_TC:
                        plugin_name = ""
