...
def main():
    ...
    # — Hermes pre_llm_call format: {"hook_event_name": "pre_llm_call", "extra": {...}}
        ...
        # ── Resolve sender (individual JID) ───────────────────────────────────
        sender = (extra.get("user") or session_key).replace("+", "")
        if "whatsapp:dm:" in sender:
            sender = sender.split("whatsapp:dm:")[1]
        elif "whatsapp:group:" in sender:
            # session_key format: agent:main:whatsapp:group:<GROUP_JID>[:<SENDER>]
            # extract only the sender part (after the group JID)
            group_part = sender.split("whatsapp:group:")[1]
            parts = group_part.split(":")
            # parts[0] = group JID (may include @g.us), parts[1] = sender number (optional)
            sender = parts[1] if len(parts) > 1 else parts[0]

        # ── Resolve chat_id: use group JID when message comes from a group ────
        effective_chat_id = ""
        if "whatsapp:group:" in session_key:
            # Extract the group JID: everything between 'whatsapp:group:' and the
            # optional ':sender' suffix — e.g. '120363xxx@g.us'
