...
def refresh_token(env):
    ...
    if not rt:
        print('⚠️  No refresh token found. Run: python3 scripts/auth.py to link Google Contacts', file=sys.stderr, flush=True)
        return ""
    ...
    try:
        ...
        print(f'⚠️  Token refresh failed: {err_desc}. Run: python3 scripts/auth.py to re-authenticate', file=sys.stderr, flush=True)
        return ""
    except Exception as e:
        print(f'⚠️  Token refresh failed: {e}. Run: python3 scripts/auth.py to re-authenticate', file=sys.stderr, flush=True)
        return ""
...
def google_get(url, token):
    ...
    try:
        ...
        try:
            ...
            err_msg = str(e)
        print(f"⚠️  Google API Error: {err_msg}", file=sys.stderr, flush=True)
        return None, False
    except Exception as e:
        print(f"⚠️  Google API Error: {e}", file=sys.stderr, flush=True)
        return None, False
...
def main():
    ...
    if cmd == "search":
        ...
        if token:
            ...
            out({"ok": False, "error": "AUTH_FAILED",
                 "message": "Could not connect to Google Contacts. The authentication tokens may be expired or invalid.",
                 "action": "Run: python3 scripts/auth.py to re-authenticate with Google."})
    else:
