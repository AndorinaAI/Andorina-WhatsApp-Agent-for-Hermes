...
import sys, json, re, time, urllib.request, fcntl
...
def save_agenda(agenda):
    try:
        ...
        tmp.write_text(json.dumps(agenda, indent=2, ensure_ascii=False), encoding="utf-8")
...
def locked_update_agenda(action_func):
    ...
    AGENDA_FILE.parent.mkdir(parents=True, exist_ok=True)
    lock_file = AGENDA_FILE.with_suffix(".lock")
...
def cmd_auto_schedule(chat_id: str, time_str: str, message: str,
    ...
    if not success:
        out({"ok": False, "error": error_msg}); sys.exit(1)
