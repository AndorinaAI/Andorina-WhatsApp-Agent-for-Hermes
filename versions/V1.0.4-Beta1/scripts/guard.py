...
DANGEROUS_PATTERNS = [
    ...
    r"\bcat\s+[/\.~]",  # cat followed by path-like argument
...
def cmd_check(number, message, msg_type="text"):
    ...
    if number.endswith("@g.us"):
        # Allow override via env var (takes precedence over .env file)
        wake_words_raw = os.environ.get("WHATSAPP_GROUP_WAKE_WORDS") \
                         or env.get("WHATSAPP_GROUP_WAKE_WORDS", "@andorina, andoriña, andorina")
        wake_words = [w.strip().lower() for w in wake_words_raw.split(",")]
        msg_l_check = message.lower()
        if not any(w in msg_l_check for w in wake_words):
            out({"allowed": False, "mode": "blocked", "reason": "group_ignored"})
            return
    ...
    if cmd == "check":
        ...
        if from_inbox:
            try:
                ...
                inbox = inbox_data if isinstance(inbox_data, list) else []
                ...
                for m in reversed(inbox):
                    if sender_raw in m.get("chatId", "") or sender_clean in clean_number(m.get("from", "")):
                        ...
                        break
            except Exception: message_parts = [""]
