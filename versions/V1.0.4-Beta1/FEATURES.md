# 🌟 Andoriña v1.0.4-Beta1 — Complete Feature Reference
## 🇬🇧 English | 🇪🇸 Español
...
- **Important Note:** History is NOT retroactive. It only begins recording from the moment the skill is installed and active.
...
- **Deep History Context:** `inbox.py read <chatId> [limit]` — allows the AI to read a specific amount of past messages (e.g., 50, 200, or `all`).
...
- **Group Mention Mode:** In groups, the bot is completely silenced to prevent API cost spikes unless explicitly mentioned by a configurable wake word (e.g. `@andorina`).
- **Semantic Topic Alerts:** You can configure the bot to listen to specific groups/chats and forward you alerts if certain keywords (topics) are mentioned, via `alerts.py`.
...
- **Nota Importante:** El historial NO es retroactivo. Solo comienza a grabarse desde el momento en que se instala y activa la skill.
...
- **Historial Profundo:** `inbox.py read <chatId> [limit]` — permite a la IA leer una cantidad específica de mensajes pasados (ej. 50, 200, o `all`).
...
- **Modo de Mención en Grupos:** En los grupos, el bot está completamente silenciado para evitar gastos de API a menos que se le mencione con una palabra de despertar (ej. `@andorina`).
- **Alertas Semánticas por Temas:** Puedes configurar al bot para que escuche ciertos grupos/chats y te reenvíe alertas si detecta ciertas palabras clave (temas), a través de `alerts.py`.
...
---
*Developed with ❤️ by Jorge. — Andoriña v1.0.4-Beta1*
